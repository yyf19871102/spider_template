...
const {ERROR_OBJ}   = require('./constants');
...
exports.spread = (obj, required, unrequired) => {
	...
	for (let key in obj) {
		...
		if (required && required.indexOf(key) > -1 || !required && (!unrequired || unrequired && unrequired.indexOf(key) < 0)) {
			// 去掉头尾的空格以及\r \n \t
			typeof value === 'string' && (value = value.replace(/\r/g, '&#114;').replace(/\n/g, '&#110;').replace(/\|/g, '&#124;'));
			raw += `${value || ''}|`;
		}
	}
...
exports.Progress = class {
	constructor(sumCount, successCount = 0, failedCount = 0, interval = 100, logger) {
		this.sumCount = sumCount;
		this.successCount = successCount;
		this.failedCount = failedCount;
		this.interval = interval;
		this.log = logger && logger.info && typeof logger.info === "function"? logger.info : console.log;
	...
	_show() {
		...
		if (this.counter >= this.interval || completeCount === 0 || completeCount === this.sumCount) {
			...
			this.log(`   |进度：${completeCount}/${this.sumCount}|完成比例：${completeRatio}%|失败比例：${failedRatio}`);
		}
	}
